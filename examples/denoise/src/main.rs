...
use wgpu::{wgt::{BufferDescriptor, DeviceDescriptor, PollType}, Backends, BufferAddress, BufferUsages, Instance, InstanceDescriptor};
...
fn main() {
    ...
    for adapter in instance.enumerate_adapters(Backends::all()) {
        if let Ok(dev) = block_on(oidn_wgpu_interop::Device::new(&adapter, &DeviceDescriptor::default())) {
    ...
    let shared_buffer = device.allocate_shared_buffers(image_byte_size as BufferAddress).unwrap();
    ...
    encoder.copy_buffer_to_buffer(&buffer, 0, shared_buffer.wgpu_buffer(), 0, image_byte_size as u64);
    ...
    // filter
    filter.filter_in_place_buffer(&shared_buffer.oidn_buffer()).unwrap();
    ...
    let image: ImageBuffer<Rgb<f32>, _> = ImageBuffer::from_raw(image.width(), image.height(), bytemuck::cast_slice(&*mapped_range)).unwrap();

    let mut file = File::create("./examples/denoise/box_denoised.png").unwrap();
    ConvertBuffer::<ImageBuffer<Rgb<u8>, _>>::convert(&image).write_to(&mut file, image::ImageFormat::Png).unwrap();
